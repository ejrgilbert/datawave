...
function log_failure() {
    log "[ERROR] Error while removing files matching the following criteria ($5 were left after removal):" "$@"
}
...
function remove_old_files() {
    ...
    if [[ ${__path_location} == ${LOC_HDFS} ]]; then
        ...
        if [[ -z ${__gender} ]]; then
            echo "find ${__path}/* -type ${__file_type} -mtime +${__day_cutoff} ${__delete_cmd} 2>/dev/null >> ${LOG_FILE}"
#            find ${__path}/* -type ${__file_type} -mtime +${__day_cutoff} ${__delete_cmd} 2>/dev/null >> ${LOG_FILE}
        else
            echo "pdsh -f 6 -b -g \"${__gender}\" \"find ${__path}/* -type ${__file_type} -mtime +${__day_cutoff} ${__delete_cmd}\"  2>/dev/null >> ${LOG_FILE}"
